...
public class AchievementController : MonoBehaviour
{
    ...
    [SerializeField] public List<AchievementData> _achievementList;
    ...
    public void UnlockAchievement(AchievementType type, string value, float gold = 0) {
        ...
        if(achievement == null) {
            try {
                if(gold > float.Parse(_achievementList.Find(a => a.Type == type).Value)) {
                    
                    achievement = _achievementList.Find(a => a.Type == type);
                }
            }
            catch {
                Debug.Log("value can't be convert into float");
            }
        }
